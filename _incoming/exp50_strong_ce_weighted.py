#!/usr/bin/env python3
"""
Experiment 50A: TTT on RealWorldQA — push image_first using disagreement-weighted
strong-branch self-training.

Idea:
  - image_first is the stronger baseline
  - question_first is the weaker ordering
  - we NEVER optimize question_first directly
  - instead, question_first is only used to measure prompt-order instability
  - we then sharpen the image_first branch itself
...
Per question:
  ...
  3. Take N_STEPS updates on layers 0-6 ONLY (top 1/4 of 28 layers)
     with loss = JS * CE(image_first, pseudo_label_from_image_first)
     optionally gated by strong confidence
  4. Evaluate both orderings after each step
  5. Reset weights before next question

This is the simplest faithful version of "use weak ordering to push the strong baseline".
"""
...
CONF_THRESH    = 0.45                # gate strong pseudo-label reinforcement
JS_MIN         = 0.00                # allow all by default; set >0 to skip tiny disagreements
EPS            = 1e-8

LAYER_FN = lambda n: any(f"language_model.layers.{l}." in n for l in range(N_LAYERS_TTT))

SWEEP_CONFIGS = [
    ("img_ce_js_lr1e-4", 1e-4, 0.9),
]
ALL_CONFIGS = ["baseline", "img_ce_js_lr1e-4"]
...
def write_metadata(result_dir, config_name, args):
    ...
    meta = {
        "experiment": "exp50_strong_ce_weighted",
        "config": config_name,
        ...
        "objective": "loss = JS(strong,weak) * CE(strong, pseudo_from_strong)",
...
def ttt_worker(gpu_id, questions, img_root, result_dir, active_configs, conf_thresh, js_min):
    ...
    results = {cfg[0]: {"qf": [], "img": [], "js": [], "conf": [], "did_update": []} for cfg in active_configs}
    ...
    for qi, q in enumerate(tqdm(questions, desc=f"[GPU {gpu_id}] {cfg_label}", position=gpu_id, leave=True)):
        ...
        for cfg_lbl, lr, momentum in active_configs:
            ...
            qf_steps, img_steps, js_steps, conf_steps, upd_steps = [], [], [], [], []
            ...
            try:
                for step in range(N_STEPS + 1):
                    ...
                    with torch.no_grad():
                        ...
                        if qi < 5:
                            print(
                                ...
                                f"gate={gate} img_ok={img_steps[-1]} qf_ok={qf_steps[-1]}",
                                flush=True,
                            )
                    ...
                    # build grads on strong branch only
                    model.train()
                    opt.zero_grad()
                    logits = model(**img_gpu).logits[0, -1].float()
                    target_id = torch.tensor([letter_ids[pred_s]], dtype=torch.long, device=device)
                    loss = js * F.cross_entropy(logits.unsqueeze(0), target_id)
                    loss.backward()
                    ...
                    if qi < 5:
                        print(
                            ...
                            f"pseudo_strong={pred_s} conf={conf_s:.4f} js={js:.6f}",
                            flush=True,
                        )
                    del img_gpu, logits, target_id, loss
                    torch.cuda.empty_cache()
            ...
            print(
                ...
                f"n_updates={sum(upd_steps[:-1])} "
                f"running img_last={np.mean([x[-1] for x in results[cfg_lbl]['img']]):.3f}",
                flush=True,
            )
...
def make_figure(summary, out_dir, n, baseline_img, baseline_qf):
    ...
    fig.suptitle(
        ...
        f"Strong-targeted TTT: disagreement-weighted CE on image_first",
...
def consolidate(args):
    ...
    merged = {cfg[0]: {"qf": [], "img": [], "js": [], "conf": [], "did_update": []} for cfg in SWEEP_CONFIGS}
    for fname in tfiles:
        with open(os.path.join(result_dir, fname)) as f:
            data = json.load(f)
    ...
    target = N_STEPS + 1
    summary = {}
    print(f"\n{'Config':<20} {'metric':<8} {'peak':>7} {'@step':>6} {'delta':>10}")
    print(f"  {'-'*55}")
    for cfg_lbl, _, _ in SWEEP_CONFIGS:
        ...
        summary[cfg_lbl] = {
            ...
            "conf": conf_m.tolist(), "did_update": upd_m.tolist(),
        }
        for metric, arr, base in [("img", img_m, baseline_img), ("qf", qf_m, baseline_qf)]:
            ...
            print(f"  {cfg_lbl:<20} {metric:<8} {peak:>7.4f} {step:>6} {peak-base:>+10.4f}{marker}")
        print(f"  {cfg_lbl:<20} {'js@0->end':<8} {js_m[0]:>7.4f} {'':>6} {js_m[-1]-js_m[0]:>+10.4f}")
        print(f"  {cfg_lbl:<20} {'upd/step':<8} {np.mean(upd_m[:-1]):>7.4f}")
...
def main():
    ...
    ap.add_argument("--result_dir", default="outputs/exp50_strong_ce_weighted")
    ap.add_argument("--out_json", default="outputs/exp50_strong_ce_weighted_summary.json")
    ap.add_argument("--out_dir", default="outputs/exp50_strong_ce_weighted_figures")
